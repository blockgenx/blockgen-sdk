...
```sh
go install github.com/blockgenx/blockgen-sdk/cosmovisor/cmd/cosmovisor@latest
```
...
```sh
go install github.com/blockgenx/blockgen-sdk/cosmovisor/cmd/cosmovisor@v0.1.0
```
...
Next, we can add a migration - which is defined using `x/upgrade` [upgrade plan](https://github.com/blockgenx/blockgen-sdk/blob/main/docs/core/upgrade.md) (you may refer to a past version if you are using an older Cosmos SDK release). In a migration we can do any deterministic state change.
